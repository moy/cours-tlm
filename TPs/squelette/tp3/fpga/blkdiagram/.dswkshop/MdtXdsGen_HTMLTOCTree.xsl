...
<TABLE BGCOLOR="{$toc_col_}" WIDTH="{$toc_width_}" COLS="2" cellspacing="0" cellpadding="3" border="0">
	...
	<TD COLSPAN="1" width="40%" align="left">
		...
		<xsl:if test="MHSINFO/MODULES/MODULE[@MODCLASS='PROCESSOR']">
		...
		<SPAN class="branch" id="Processors">		
			<xsl:for-each select="MHSINFO/MODULES/MODULE[@MODCLASS='PROCESSOR']">
				...
				<A HREF="{$trg_html_}#_{@INSTANCE}" style="text-decoration:none"><SPAN style="color:{$DS_COL_XPRP}; font: normal 14px Verdana Arial,Helvetica,sans-serif">&#160;&#160;&#160;<xsl:value-of select="@INSTANCE"/></SPAN></A><BR></BR>
				<xsl:if test="MEMORYMAP">
					<A HREF="{$trg_html_}#_{@INSTANCE}_MemoryMap" style="text-decoration:none"><SPAN style="color:{$DS_COL_BLACK}; font: italic 14px Verdana Arial,Helvetica,sans-serif">&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;memory map</SPAN></A><BR></BR>
				</xsl:if>
			</xsl:for-each>
		</SPAN>	
		</xsl:if>
		...
		<SPAN class="branch" id="Peripheral">
			<xsl:for-each select="MHSINFO/MODULES/MODULE[@MODCLASS='PERIPHERAL']">
				<xsl:sort select="@INSTANCE"/>
				<A HREF="{$trg_html_}#_{@INSTANCE}" style="text-decoration:none"><SPAN style="color:{$DS_COL_XPRP}; font: normal 14px Courier Verdana Arial,Helvetica,sans-serif">&#160;&#160;&#160;<xsl:value-of select="@INSTANCE"/></SPAN></A><BR></BR>
			</xsl:for-each>
		</SPAN>	
		</xsl:if>		
